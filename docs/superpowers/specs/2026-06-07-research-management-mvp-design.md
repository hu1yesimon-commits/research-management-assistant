...
- Advanced-lite 模式：根据历史对话、实验日志、已读论文摘要，生成 2-5 个可探索方向和搜索 query。
...
2. `rewrite_query`
   ...
   - Advanced-lite 模式用 LLM Rewriter 生成可检索方向，例如轻量化、可解释性、模块结合、损失函数改造、数据增强等。
...
- `papers`
  ...
  - `status` 可为 `candidate`、`accepted`、`uploaded`、`embedded`、`rejected`。
...
- `candidate` 状态不代表已入库。
- 只有 `uploaded` 或 `embedded` 状态的论文 DOI 才进入强去重集合。
