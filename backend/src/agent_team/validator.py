...
class PlanValidator:
    def validate(
        ...
        *,
        ...
        if plan.plan_type in {"idea", "research_then_idea"} and experiment_log is None:
        ...
        if plan.plan_type == "clarify":
            ...
            raise PlanValidationError(
                "needs_clarification is forbidden for non-clarify plans"
            )
...
def validate_plan(plan: LeaderPlan, planner_input: PlannerInput) -> LeaderPlan:
    return PlanValidator().validate(plan, experiment_log=planner_input.experiment_log)
